...
import {
  ...
  ITrackingStateManagerInternal,
} from '../ddd-core/impl/tracking-state-transition';

/**
 * Representa un mecanismo de seguimiento para detectar cambios en objetos.
 * Gestiona el estado de tracking de una entidad (isNew, isDirty, isDeleted, etc.).
 *
 * Aplica principios SOLID:
 * - SRP: Solo gestiona el estado interno de tracking
 * - OCP: Extensible mediante inyección de IChangeDetector
 * - DIP: Depende de abstracciones (IChangeDetector) no de implementaciones concretas
 */
export class TrackingStateManager
  implements ITrackingStateManager, ITrackingStateManagerInternal
{
